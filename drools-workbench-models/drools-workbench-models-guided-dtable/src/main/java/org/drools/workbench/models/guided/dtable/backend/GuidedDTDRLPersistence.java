...
public class GuidedDTDRLPersistence {
    ...
    private IPattern findByFactPattern(List<IPattern> patterns,
                                       Pattern52 find) {
        final boolean isNegated = find.isNegated();

        if (isNegated) {
            return doFindByNegatedFactPattern(patterns,
                                              find);
        } else {
            return doFindByFactPattern(patterns,
                                       find);
        }
    }

    private IPattern doFindByFactPattern(final List<IPattern> patterns,
                                         final Pattern52 find) {
        final String boundName = find.getBoundName();
    ...
    private IPattern doFindByNegatedFactPattern(final List<IPattern> patterns,
                                                final Pattern52 find) {
        final String factType = find.getFactType();
        for (IPattern ifp : patterns) {
            if (ifp instanceof FactPattern) {
                final FactPattern fp = (FactPattern) ifp;
                final Optional<IPattern> match = checkNegatedFactPattern(fp,
                                                                         factType);
                if (match.isPresent()) {
                    return match.get();
                }
    ...
    private Optional<IPattern> checkNegatedFactPattern(final FactPattern fp,
                                                       final String factType) {
        if (fp.isNegated()) {
            if (fp.getFactType().equals(factType)) {
